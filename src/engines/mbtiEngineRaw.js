// ══════════════════════════════════════════════════════════════
// PANDA FORTUNE — MBTI 相性エンジン
// 元 panda-fortune-paid.html 207-510 行 <script id="mbti-engine"> をそのまま ESM 化
// 副作用で window.calcMbtiScore / window.MBTI_ENGINE_READY などに代入される
// ══════════════════════════════════════════════════════════════
(function(){
...
// タイプ文字列 → インデックス（高速検索用）
const TYPE_INDEX = {};
MBTI_TYPES.forEach((t, i) => { TYPE_INDEX[t] = i; });
 ...
 */
function getMbtiLevel(typeA, typeB) {
 ...
 */
function getMbtiCompatibility(typeA, typeB) {
  const a = normalizeMbtiType(typeA);
  const b = normalizeMbtiType(typeB);
 ...
 */
function getMbtiCompatChart(type) {
  const a = normalizeMbtiType(type);
  if (a === null) return null;

  return MBTI_TYPES.map(b => {
    const level = COMPAT_MATRIX[TYPE_INDEX[a]][TYPE_INDEX[b]];
 ...
 * compat-logic.js の calcMbtiScore() の代替として使用可能
 * @param {string} typeA
 * @param {string} typeB
 * @returns {number} 0-99
 */
function getMbtiScoreForCompat(typeA, typeB) {
 ...
 *
 * // Node.js
 * const { getMbtiCompatibility } = require('./mbti-engine');
 * const result = getMbtiCompatibility('INFP', 'ENFJ');
 * console.log(result);
 * // → { typeA:'INFP', typeB:'ENFJ', level:3, label:'とても相性が良い', score:92, ... }
 *
 * // compat-logic.js との連携
 * // calcMbtiScore() の代わりに getMbtiScoreForCompat() を使用:
 * //   const mbtiScore = getMbtiScoreForCompat(myMbti, ptMbti);
 *
 * // 全タイプとの相性一覧
 * const chart = getMbtiCompatChart('INTJ');
 * chart.forEach(c => console.log(`${c.type}: ${c.label}`));
 *
 * ── compat-logic.js への組み込み方法 ─────────────────────
 *
 * compat-logic.js の calcMbtiScore() を以下に差し替え:
 *
 *   function calcMbtiScore(myMbti, ptMbti) {
 *     return getMbtiScoreForCompat(myMbti, ptMbti);
 *   }
 *
 * これにより、4段階ルックアップテーブルベースの
 * 精密なMBTIスコアが適用されます。
 */

// ── PANDA FORTUNE ブリッジ ──────────────────────────────────────
var _MBTI_SCORE_MAP_4 = {0:48, 1:64, 2:79, 3:95};

window.calcMbtiScore = function(typeA, typeB) {
  var level = getMbtiLevel(typeA, typeB);
  if (level < 0) return 70;
  return _MBTI_SCORE_MAP_4[level];
};

window.getMbtiInfo = function(typeA, typeB) {
  var result = getMbtiCompatibility(typeA, typeB);
  if (!result) return null;
  result.scoreCustom = _MBTI_SCORE_MAP_4[result.level];
  return result;
};

window.MBTI_ENGINE_READY = true;
